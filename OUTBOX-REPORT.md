...
*Thompson's value appears to scale with relay graph complexity. Profiles with 400-1,800 follows see the largest gains (+55-60% relative at 1yr for hodlbod, jb55, ODELL) — the relay graph is diverse enough that random selection consistently misses good relays. The smallest profile (fiatjaf, 194 follows) sees near-zero gain: ~140 unique relays after filtering means the 20-connection budget already samples most combinations, leaving little room to learn. Whether this pattern holds broadly, and where the crossover lies, is being tested with 6 additional JP profiles (84–1,746 follows).*
