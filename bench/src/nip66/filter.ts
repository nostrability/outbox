...
export interface FilterResult {
  /** Relays confirmed alive by at least one source. */
  knownAlive: Set<RelayUrl>;
  /** Relays removed (not seen by any source). */
  removed: Set<RelayUrl>;
  /** .onion relays preserved without checking. */
  onionPreserved: number;
  /** URLs that failed to parse, preserved. */
  parseFailedPreserved: number;
  /** Authors whose entire relay set was removed (read-only count). */
  filteredToEmptyAuthors: number;
}
