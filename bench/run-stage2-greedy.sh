...
# Algorithms: greedy (base) + greedy-thompson (paired for delta variance)
...
PROGRESS_FILE=".cache/stage2_progress.log"
touch "$PROGRESS_FILE"

echo "=== Stage 2: Greedy+Thompson EN (outbox-uzu) ==="
echo "Algorithms: $ALGOS"
echo "Profiles: 6 EN × 2 windows × $SESSIONS sessions = 60 runs"
echo "start=$(date +%s)" > ".cache/stage2_timestamps.txt"
echo "Start: $(date)"
echo

for WINDOW in $WINDOWS; do
  COMMON="--verify --verify-window $WINDOW --verify-concurrency 10 --nip66-filter liveness --no-phase2-cache --fast --output both"
  LOGDIR=".cache/stage2_greedy_logs/${WINDOW}"
  mkdir -p "$LOGDIR"

  # Resume-safe score clearing
  MARKER=".cache/stage2_${WINDOW}_scores_cleared"
  if [ ! -f "$MARKER" ]; then
    echo "Clearing greedy-thompson Thompson scores for window=$WINDOW..."
  ...
  for session in $(seq 1 $SESSIONS); do
    for name in $NAMES; do
      eval "pk=\$PK_${name}"
      key="stage2_${WINDOW}_${name}_s${session}"
      ...
      if [ ${PIPESTATUS[0]} -eq 0 ]; then
        ...
        grep -iE '(Greedy Set-Cover|Greedy\+Thompson)' "$logfile" | grep -iE 'Recall' | head -2
      else
...
echo "end=$(date +%s)" >> ".cache/stage2_timestamps.txt"
