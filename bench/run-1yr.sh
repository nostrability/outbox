#!/usr/bin/env bash
# 1yr Thompson benchmark: 6 profiles × 5 sessions, genuine (--no-phase2-cache)
# Includes greedy + welshman baselines for comparison
set -uo pipefail

ALGOS="greedy,welshman,welshman-thompson,fd-thompson"
...
for session in $(seq 1 $SESSIONS); do
  for name in $NAMES; do
    ...
    grep -E '(Greedy|Weighted Stochastic|Welshman|FD\+)' "$logfile" | grep -E 'Recall' | head -4
    echo
    sleep 30
  done
